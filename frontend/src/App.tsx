...
export default function App() {
  ...
  async function bootstrap() {
    try {
      const [cats, exps, dailyStats, dash] = await Promise.all([
        getCategories(OWNER_ID),
        getExpenses(OWNER_ID),
        fetchDaily(OWNER_ID),
        fetchDashboard(OWNER_ID),
      ]);
      setCategories(cats);
      setExpenses(exps);
      setDaily(dailyStats);
  ...
  async function refreshStats() {
    const [dailyStats, dash] = await Promise.all([fetchDaily(OWNER_ID), fetchDashboard(OWNER_ID)]);
    setDaily(dailyStats);
    setSummary(dash);
  }
  ...
  return (
    <div className="page">
      <header className="hero">
        ...
        <div className="pill">FastAPI · Postgres · Vite</div>
      </header>
